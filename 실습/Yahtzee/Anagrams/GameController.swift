...
class GameController: TileDragDelegateProtocol {
    ...
    init() {
        ...
        audioController.preloadAudioEffects(audioFileName: AudioEffectFiles)
    }
    ...
    func placeTile(tileView: TileView, targetView: TargetView) {
        ...
        //placeTile() 메소드 맨 뒤에 타일을 놓은 중앙에 폭발뷰 생성
    ...
    func checkForSuccess() {
        ...
        UIView.animate(withDuration: 3.0, delay: 0.0, options: UIViewAnimationOptions.curveEaseOut, animations: {stars.center = CGPoint(x: endX, y: startY)}, completion: {(value:Bool) in stars.removeFromSuperview()})
        print("Game Over!")
    }
